...
async function executeTask(task) {
  ...
  log('EXECUTE', `  amount       : ${task.amount} USDCx`);
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && url.pathname === '/api/tasks/register') {
      try {
        ...
        log('REGISTER', `One-time task ${taskId} | trigger: ${triggerBlock} | amount: ${amount} USDCx`);
    ...
    if (req.method === 'POST' && url.pathname === '/api/tasks/register-recurring') {
      try {
        const body = await readBody(req);
        const { taskId, recipient, amountPerExecution, firstTriggerBlock, intervalBlocks, maxExecutions } = body;
        ...
        log('REGISTER', `Recurring task ${taskId} | first_trigger: ${firstTriggerBlock} | interval: ${intervalBlocks} | max: ${maxExecutions} | amount: ${amountPerExecution} USDCx`);
        return json({ ok: true, task: serializeTask(taskStore.get(taskId)) });
      } catch (err) {
        return json({ error: err.message }, 400);
      }
    ...
    if (req.method === 'POST' && url.pathname === '/api/tasks/register-conditional') {
      try {
        const body = await readBody(req);
        const { taskId, recipient, amount, triggerBlock, conditionType, conditionValue } = body;
        ...
        log('REGISTER', `Conditional task ${taskId} | trigger: ${triggerBlock} | condition: ${conditionType === CONDITION_TYPES.PRICE_ABOVE ? 'price_above' : 'price_below'} ${conditionValue} | amount: ${amount} USDCx`);
        return json({ ok: true, task: serializeTask(taskStore.get(taskId)) });
      } catch (err) {
        return json({ error: err.message }, 400);
      }
    ...
    if (req.method === 'POST' && url.pathname === '/api/tasks/register-escrow') {
      try {
        const body = await readBody(req);
        const { taskId, recipient, amount, triggerBlock, requiredApprovals } = body;
        ...
        log('REGISTER', `Escrow task ${taskId} | trigger: ${triggerBlock} | required_approvals: ${requiredApprovals} | amount: ${amount} USDCx`);
        return json({ ok: true, task: serializeTask(taskStore.get(taskId)) });
      } catch (err) {
        return json({ error: err.message }, 400);
      }
...
async function main() {
  console.log('');
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║        USDCx Automation Keeper Bot                         ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log('');
