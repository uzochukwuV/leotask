...
/**
 * Leotask - Comprehensive Test Suite
 * ===================================
 * Tests all features: one-time, recurring, conditional, multi-party escrow, USDCx
 *
 * Usage:
 ...
 *   node test_all_features.mjs --usdcx            # Test USDCx support only
...
const CONFIG = {
  ...
  testAmount:        parseInt(process.env.TEST_AMOUNT || '1000000'),  // 1 ALEO in microcredits
...
async function testOneTimeTransfer() {
  logSection('TEST: One-Time Scheduled Transfer');
  ...
  log('ONE-TIME', `  Amount: ${CONFIG.testAmount} microcredits`);
...
async function testRecurringTransfer() {
  logSection('TEST: Recurring Transfer');
  ...
  log('RECURRING', `  Amount per execution: ${amountPerExecution} microcredits`);
  log('RECURRING', `  Total amount: ${totalAmount} microcredits`);
...
async function testConditionalTransfer() {
  logSection('TEST: Conditional Transfer');
  ...
  log('CONDITIONAL', `  Amount: ${CONFIG.testAmount} microcredits`);
...
async function testMultiPartyEscrow() {
  logSection('TEST: Multi-Party Escrow');
  ...
  log('ESCROW', `  Amount: ${CONFIG.testAmount} microcredits`);
...
async function testUSDCxSupport() {
  logSection('TEST: USDCx Support');

  const currentBlock = await fetchBlockHeight();
  const triggerBlock = currentBlock + 10n;
  const taskId = generateTaskId();

  log('USDCX', `Creating USDCx transfer task ${taskId}`);
  log('USDCX', `  Amount: ${CONFIG.testAmount} microcredits`);
  log('USDCX', `  Token type: USDCx (1)`);
  log('USDCX', `  Trigger block: ${triggerBlock}`);
  ...
  try {
    ...
    if (result.ok) {
      logSuccess('USDCX', `Task registered successfully`);
      log('USDCX', `  Task ID: ${result.task.taskId}`);
      log('USDCX', `  Token type: ${result.task.tokenType}`);
      log('USDCX', `  Blocks remaining: ${result.task.blocksRemaining}`);
      return { success: true, taskId };
    } else {
      logError('USDCX', `Registration failed: ${result.error}`);
      return { success: false, error: result.error };
    }
  } catch (err) {
    logError('USDCX', `Exception: ${err.message}`);
    return { success: false, error: err.message };
  }
}
...
async function main() {
  console.log('');
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║        Leotask - Comprehensive Test Suite                  ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log('');
  ...
  if (runAll || args.includes('--usdcx')) {
    results.usdcx = await testUSDCxSupport();
    await sleep(2000);
  }
