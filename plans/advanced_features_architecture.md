# Leotask Advanced Features Architecture

## Overview

This document describes the advanced features implemented in Leotask v4, including recurring payments, conditional transfers, multi-party escrow, and USDCx support.
...
2. Funds are escrowed in the program's public balance
3. User receives a `CancelAuth` record for cancellation
4. Keeper executes when trigger block is reached
5. Funds are transferred to recipient

**Smart Contract Functions:**
- `create_scheduled_transfer()` - Create a one-time transfer
...
3. User receives a `RecurringAuth` record for cancellation
...
- `cancel_recurring_transfer()` - Cancel remaining executions
...
**Example:**
```javascript
// Pay 100 USDC every 100 blocks for 10 times
{
  taskId: "0x123",
  recipient: "aleo1...",
  amountPerExecution: 100000000,  // 100 USDC in microcredits
...
3. User receives a `ConditionalAuth` record for cancellation
...
- `cancel_conditional_transfer()` - Cancel before execution
...
**Example:**
```javascript
// Sell 1000 USDC when price drops below 0.95
{
  taskId: "0x456",
  recipient: "aleo1...",
...
7. If not approved, creator can cancel and reclaim funds
...
**Example:**
```javascript
// Transfer 10000 USDC, requires 3 out of 5 approvals
{
  taskId: "0x789",
  recipient: "aleo1...",
...
**Description:** Support for USDCx (Superfluid wrapper of USDC) tokens.

**Implementation:**
- Token type is specified using `token_type` field:
  - `0` = ALEO (native token)
  - `1` = USDCx (Superfluid token)
- Token program addresses are stored in `token_programs` mapping
- Admin can update token program addresses using `update_token_program()`

**Smart Contract Functions:**
- `update_token_program()` - Update token program address (admin only)

**API Endpoints:**
- All task registration endpoints accept `tokenType` parameter
...
```leo
mapping roles: u8 => address;                    // Admin and keeper roles
mapping task_data: field => TaskInfo;            // Task information
mapping escrow_approvals: field => bool;         // Escrow approvals
mapping escrow_parties: field => bool;           // Escrow parties
mapping token_programs: u8 => address;           // Token program addresses
```
...
node test_all_features.mjs --usdcx            # Test USDCx only
node test_all_features.mjs --stress           # Stress test
```

### Edge Case Tests (`test_edge_cases.mjs`)
...
Leotask v4 provides a comprehensive automation platform for Aleo blockchain with support for:
